...
```bash
npm install @marc-newton/solid-class-js
```

```bash
pnpm add @marc-newton/solid-class-js
```

```bash
yarn add @marc-newton/solid-class-js
```

```bash
bun add @marc-newton/solid-class-js
```
...
* `reset(override?: Partial<this> | any): void` - Drops all current patch modifications natively running through `.assign()` forcefully rolling the data state natively backward safely to the last explicitly `commit()`ted snapshot! Optionally accepts an `override` object to establish a new custom default baseline state.
